...
<div class="dropdown2">
  ...
  <div class="dropdown-content2">
	...
	<a href="./fuel_log.php?choice=4" target="_blank">Fuel Log</a>
		...
		<a href="./alarm_history.php" target="_blank">Distress History</a> <a href="./distress_signals.php" id="distress" target="_blank">Distress</a>
</div>
<style>
.blinkRed {
		...
		<center><h1><b>Update Details of <?php echo $_GET['choice']?></b></h1> </center>

	<br />
	<div id="stylized" class="myform">

	<form id="driver_form" action="driver_details.php" method="POST">
		...
		<input type="text" id="alternative_phone" value="">
	  
		<label>Next of Kin Name</label>
		<input type="text" id="kin">
	
		<label>Next of Kin Phone Number </label>
		<input type="text" id="kin_phone" value="">
	 
		<label>Blood Group</label>
		<input type="text" id="blood_group" value="">
	   
		<label>Date of Join:</label>
		<input type="text" id="join_date" value="">
...
<script>
    var database = firebase.database().ref('driver/'+choice);
    database.once('value', function(snapshot){
        if(snapshot.exists()){
			var val = snapshot.val();
			$('#name').attr('value', val.name);
			$('#username').attr('value', choice);
			$('#phone ').attr('value', val.phone );
			$('#alternative_phone ').attr('value', val.alternativephone );
			$('#kin ').attr('value', val.kin );
			$('#kin_phone ').attr('value', val.kin_phone );
			$('#blood_group').attr('value', val.blood_group);
			$('#dob').attr('value', val.dob);
			$('#join_date').attr('value', val.join_date);
			$('#religion').attr('value', val.religion);
			$('#password').attr('value', val.password);
        }
    });
	...
	$('#driver_form').submit(function(evt) {
		...
		if(choice != username){
			firebase.database().ref('/driver/'+choice).remove();
		}
